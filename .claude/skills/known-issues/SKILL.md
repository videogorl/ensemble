...
- **Current rule:** Ordinary short rename/create/filter flows should use plain `.sheet` presentation again. Parent screens may suppress only their own active navigation/search chrome while presenting a local modal if logs prove that parent chrome is the loop source. Context-menu album/artist/track metadata editors also use normal sheets backed by the shared `TextInputView`; the previous root `phoneSafeAuxiliaryPresentation(...)` path reproduced the same `NavigationBarContentView` loop when album rename was focused.

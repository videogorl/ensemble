...
- Use `MetadataMutationWorkflow` for track, album, and artist metadata edit/delete UI. It builds `MetadataEditRequest`, calls the mutation service, and returns standardized toast payloads; views should only own editor presentation, confirmation dialogs, and post-delete navigation. Album card/grid context-menu rename flows should own a local `ContextMenuMetadataEditorRequest` sheet, matching their local add-to-playlist and delete presenters. Remaining shared context-menu metadata rename flows may request `ContextMenuMetadataEditorCoordinator` and let the root present the shared `TextInputView` sheet. Do not observe that root request from library/grid screens to hide local navigation or search chrome.

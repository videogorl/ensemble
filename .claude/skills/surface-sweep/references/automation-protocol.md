...
- Loading `app-policies`, routing each finding to relevant policy references, and merging policy discrepancies or missing-policy gaps.
...
- Compare observed behavior against relevant `app-policies` references. If the behavior contradicts policy, record a policy discrepancy; if no policy covers an important behavior, record a missing-policy gap.
...
- `policy_status`, `policy_references`, `policy_discrepancy`, `missing_policy`,
- `first_seen_commit`, `suspected_area`, `source_files`, `fix_hint`,
- `verification_steps`, `notes`.

Policy status values:
- `matches_policy`: observed behavior matches the relevant policy.
- `contradicts_policy`: observed behavior conflicts with a loaded policy reference.
- `missing_policy`: important observed behavior has no clear policy coverage.
- `policy_unclear`: policy exists but is ambiguous for the observed behavior.
- `not_policy_relevant`: purely visual/mechanical issue with no durable behavior policy.
...
```text
Use $surface-sweep and $app-policies for the iPhone compact slice only. Build/launch the Ensemble iPhone simulator if needed, drive every iPhone compact surface from surface-map.md, compare observed behavior against relevant app-policies references, save artifacts under <ARTIFACT_ROOT>/iphone, reproduce suspected issues once, inspect likely logs/source owners, and write <ARTIFACT_ROOT>/iphone/runner-report.md plus <ARTIFACT_ROOT>/iphone/platform-report.json. Include policy discrepancies and missing-policy gaps. Do not edit source files or complete destructive actions.
```

iPad runner:

```text
Use $surface-sweep and $app-policies for the iPad regular slice only. Build/launch the Ensemble iPad simulator if needed, drive every iPad/sidebar surface from surface-map.md, compare observed behavior against relevant app-policies references, save artifacts under <ARTIFACT_ROOT>/ipad, reproduce suspected issues once, inspect likely logs/source owners, and write <ARTIFACT_ROOT>/ipad/runner-report.md plus <ARTIFACT_ROOT>/ipad/platform-report.json. Include policy discrepancies and missing-policy gaps. Do not edit source files or complete destructive actions.
```

macOS runner:

```text
Use $surface-sweep and $app-policies for the macOS slice only. Build/launch the macOS Ensemble app if needed, use Computer Use or macOS UI tooling to touch every macOS surface from surface-map.md, compare observed behavior against relevant app-policies references, save artifacts under <ARTIFACT_ROOT>/macos, reproduce suspected issues once, inspect likely logs/source owners, and write <ARTIFACT_ROOT>/macos/runner-report.md plus <ARTIFACT_ROOT>/macos/platform-report.json. Include policy discrepancies and missing-policy gaps. Do not edit source files or complete destructive actions.
```
...
- Observed behavior contradicts an `app-policies` reference, or a user-visible behavior is important enough to preserve but has no clear policy coverage.
...
3. Merge policy discrepancies and missing-policy gaps, deduplicating by policy reference and surface.
4. Promote cross-platform issues when the same behavior appears on multiple platforms; keep platform-specific evidence paths inside one finding's `platforms` and `evidence` arrays.
5. Separate environment/data blockers from app bugs.
6. For P1/P2 issues and any policy contradiction, personally inspect the strongest artifact, policy reference, and source owner before finalizing.
7. Write the final sweep report, optional Markdown fix report, and required `fix-report.json`.

---
name: surface-sweep
description: "Use when the user asks to visually test, touch, or walk every Ensemble app surface across iOS/iPadOS and macOS, including full UI sweeps, platform parity checks, and repeatable screenshot/accessibility evidence runs."
---

# Ensemble Surface Sweep

Run this skill when asked to touch every visible surface of Ensemble or to do a broad visual regression pass across platforms.

This is a runtime verification workflow, not a unit-test replacement. It should usually be loaded with `testing`; load `simulator-test` for iOS/iPadOS, and use Computer Use or Xcode/macOS UI tooling for macOS. Load `app-policies` only when the sweep is tied to behavior policy changes.
...
- For each screen, capture at least one screenshot plus either an accessibility dump, visible UI description, or concise notes.
...
Use [evidence-template.md](references/evidence-template.md) for the final report. Keep artifacts in `/tmp`, and include paths in the handoff.
...
- visual defects or regressions with screenshot paths,
- risky actions intentionally not completed.

## Failure Handling

If build or launch fails, stop the sweep and report that blocker first. If only one platform is blocked, complete the remaining platforms and mark the blocked platform incomplete.

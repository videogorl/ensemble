...
- `LibraryViewModel` -- Library browse root state. Before publishing cached rows, it filters CoreData source rows against `AccountManager.enabledSources()`: no enabled libraries clears in-memory browse state immediately, and stale disabled-source cleanup is scheduled after the load path so artwork, lyrics, offline targets, downloads, and library/feed/mood cache are removed without blocking startup or first tab interaction.

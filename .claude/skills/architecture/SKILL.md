...
- `LibraryViewModel` -- Library browse root state. Before publishing cached rows, it reconciles CoreData source rows against `AccountManager.enabledSources()`: no enabled libraries purges all library/feed/mood cache, and stale disabled sources go through `SyncCoordinator.cleanupRemovedSource()` so artwork, lyrics, offline targets, and downloads are removed with the source.

...
Shared: EnsembleDomain + EnsembleSiriShared
...
- UI depends on Core.
- Core depends on Domain, API, Persistence, SiriShared, and Nuke.
...
| `EnsembleDomain` | Cross-client hidden-media wire identity plus Watch-portable account, media, category, and playback-status models. |
...
- `MusicSourceSyncProvider` owns library/playlist synchronization and Feed/artwork loading. Playback resolution/reporting, ratings, online playlist mutations, collection details, file metadata, lyrics, and radio are opt-in `MusicSource*` capability protocols. Rating providers receive the normalized track, own API item-identity translation, and return normalized post-success cache-reconciliation effects; shared coordinators must not branch on provider type. `SyncProviderResolver` requires the exact library provider; a valid server-scoped item such as a Plex playlist may resolve only to a provider with the requested capability on the same provider/account/server. Missing, malformed, cross-provider, and cross-server sources fail explicitly.
- `PlaylistMutationController` owns provider-neutral playlist validation and local reconciliation. The resolved `MusicSourcePlaylistMutating` provider owns remote create, add, rename, delete, replace, and membership-edit behavior; do not put provider API branches back into the controller or `SyncCoordinator`.
- `MusicItemActionAvailability` is the Core-owned contract for item-level add, rename, reorder, delete, download, favorite, and metadata-edit availability. `MusicItemActionCapabilities` carries optional provider-mapped per-item overrides; actions without an override fall back to the source-wide contract. Core encodes those overrides into Persistence-owned opaque track/album/artist payloads so permissions survive sync and relaunch without making Persistence depend on Core; nil input preserves a provider that supplies no overrides, while an encoded empty set clears them. Shared UI consumes the resolved state and reason without provider-specific branches.
- Media without a complete valid source key is invalid for routing and mutation. Reject it explicitly; do not infer or repair ownership from cached matches, the sole configured provider, or the selected playlist target.
...
- `EnsembleCore.MediaSourceIdentity` owns strict provider/account/server/library source-key parsing and scope comparison for the app. Provider item and hub IDs are not source keys and must not be truncated through this parser; missing or malformed keys never enter provider resolution.
...
- Destructive source cleanup belongs in `SourceCacheCleanupService` or the relevant persistence/offline owner, not in UI ViewModels. It removes source-owned Feed items as well as library/offline rows and that source's durable artwork. Shared bounded rendering caches are disposable and may be cleared globally, but persistent artwork identity and deletion stay source-scoped. `ArtworkDownloadManager` accepts only source-scoped durable identities and performs one startup purge of pre-v2 unscoped files before marking the cache upgraded.

...
- `PlaylistMutationWorkflow` -- Shared playlist rename/delete workflow used by playlist root, detail, and sidebar/pinned surfaces; owns title trimming, mutation outcome routing, and pending/success/failure toast payload policy while views keep local navigation, confirmation, optimistic list state, and pin updates

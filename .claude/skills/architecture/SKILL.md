...
- `SourceCacheCleanupService` -- Utility-priority source/all-library cache eviction worker. It removes downloads and offline targets before deleting library rows, clears lyrics/artwork caches, and keeps destructive cleanup out of `LibraryViewModel` and `SyncCoordinator` main-actor load paths.

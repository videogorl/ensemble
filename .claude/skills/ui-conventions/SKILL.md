...
- **iOS 15:** Use `NestedNavigationLink` only as the root `NavigationView` bridge for coordinator-driven entry into a tab or hidden More destination. Once inside a browse/detail stack, use native `NavigationLink(destination:)` for in-stack pushes so SwiftUI owns animations and back behavior. If a model-backed source card exposes a toolbar background flash during the push, drive only the iOS 15 navigation-bar appearance with `legacyNavigationBarBackground(isTransparent:)`; do not hide root toolbar/filter chrome just to clean up the transition snapshot. For artwork-backed detail heroes where `NavigationView` still reserves a white bar band, prefer a surface-local overlay chrome on iOS 15 instead of global nav-bar shims.

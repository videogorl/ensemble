...
- Watch Albums browse uses native discrete Digital Crown detents and a fixed two-cover stack, not a scrolling collection. Each detent replaces the selected album by raising its cover from the bottom; reversing the Crown lifts that cover away to reveal the previous album. Keep artwork clipped to its own viewport and the selected title in a background-free bottom lane so fast Crown input cannot leave partial covers over the title. Expose the selected album's shared alphabetical section letter through the native `digitalCrownAccessory`; let the system own its placement and transient visibility, and style its content as the same large green circular marker used by the native Artists list index. Tapping opens album detail, where album actions remain available. Do not add a page indicator, custom drag recognizer, manual scroll-offset tracking, or an album-cell `ScrollView`.

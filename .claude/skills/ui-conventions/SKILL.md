...
- **Detail loading stability:** Album, playlist, artist, and virtual collection detail loads should avoid full-screen centered loaders that later swap to top-aligned headers. Use `MediaDetailSurface.LoadingState` or keep the shared header/table shell mounted with a compact footer loader during initial data fetches. On macOS, pin the detail root from the top with an outer top-aligned container/spacer; do not force the AppKit native table host itself to infinite height as a reflow workaround.

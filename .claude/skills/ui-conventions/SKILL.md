...
- **Search chrome ownership:** In tab-based navigation, attach `.searchable` only while that tab is the active root screen, except for the short native tab handoff window where outgoing Search must keep its search controller mounted to avoid relayout before the destination appears. Collapse/remove search chrome before pushing detail or after switching away so stale `UISearchController` state doesn't leak padding, keyboard state, or toolbar behavior into pushed views or other tabs. Use SwiftUI's `dismissSearch` environment action to collapse search; do not force keyboard dismissal with `UIApplication.sendAction(...)`.
...
- **Route-owned links:** For feed cards, hub headers, and other coordinator-owned typed routes that must survive source/list refreshes, use `NavigationCoordinator.routeLink(to:)` instead of local `#available` branches or inline destination closures. `routeLink(to:)` should push through the scene-local coordinator path, not nested `NavigationLink(value:)`, so links inside More-tab destinations can keep drilling into albums, artists, playlists, and related items. When the caller already has a concrete `Album`, `Artist`, or `Playlist`, route to `.albumDetail(...)`, `.artistDetail(...)`, or `.playlistDetail(...)` instead of ID-only loaders. Context menu navigation actions should use `routeFromMenu(to:)` or `navigateFromMenu(to:)` so path updates happen after native menu dismissal.
...
- **Mini player offset:** Root chrome registration owns MiniPlayer placement. On iOS, use `TrackListLayoutMetrics.rootMiniPlayerBottomLift(safeAreaBottom:tabBarBottomClearance:)`. Root geometry already ends at the safe-area boundary, so convert the native tab bar's window-bottom clearance into root coordinates by subtracting the bottom safe-area inset before adding the shared visual gap. With no tab bar, apply no additional root lift; the safe area is already preserved by the root frame. The floating MiniPlayer adds no transparent bottom padding of its own. The existing UIKit tab-container probe provides the tab-bar measurement across iOS 15 and newer. Use `TrackListLayoutMetrics.detailMiniPlayerBottomLift(safeAreaBottom:)` for registered split/detail panes. Do not add per-screen mini-player offsets or publish root mini-player geometry from pushed destination content. iPadOS mini-player placement should resolve from root-space coordinates centered on the active content viewport: root bounds when the sidebar is closed, and root bounds minus the frozen visible leading sidebar metric when the sidebar is open. Destination views and pushed detail content must not influence root mini-player placement; only sidebar visibility/width, root rotation/size, keyboard suppression, and Now Playing presentation should move or hide the persistent shell. A missing sidebar preference during a SwiftUI navigation render pass is not a sidebar-close signal; the split owner's explicit sidebar registration should switch between the root viewport and sidebar-adjusted content viewport. Keep the persistent root mini player in the `RootSceneLayerHost` overlay across iPhone, iPadOS, and macOS so it shares the root view's rotation and safe-area coordinate space. On iOS 26+, keep the persistent iPadOS root mini-player shell on native Liquid Glass, but do not give it a shared glass-effect identity for unrelated navigation; Now Playing handoff animation should remain explicit.
...
- Mini-player transport buttons must expose explicit, state-correct accessibility labels (`Previous`, `Play`/`Pause`, and `Next`) rather than relying on SF Symbol inference.
...
- On iPadOS/macOS root sidebar layouts, keep the existing single `AuroraVisualizationView` with `.rootBackdrop` in `RootSceneLayerHost`. Do not move the live aurora into only the detail column or create a second root aurora renderer.
...
- Disabled detail-surface playback action labels should use primary text color and let the native disabled state dim the control. Do not force `onAccent`/white foreground on disabled primary actions, because macOS glass turns the disabled fill neutral gray.
...
- **iPad/macOS:** `ProfileToolbarButton` placed in sidebar toolbar, replacing the previous gear icon. On macOS, Downloads precedes Profile; the native menu hides its disclosure indicator, uses a fixed toolbar footprint, and shows the circular profile image when set, with the native profile glyph as its fallback.
...
- The button displays the user's profile image if set and otherwise falls back to a person icon. On macOS, pre-crop the local image into the toolbar bitmap because toolbar promotion does not preserve SwiftUI image crop modifiers.
...
- Watch Albums browse uses native Digital Crown detents and a fixed two-cover stack, not a scrolling collection. Ease the cover transition through the Crown's continuous offset, preserve continuity when it crosses album boundaries, and smoothly settle to the selected album when the Crown becomes idle. Slow Crown turns replace albums one at a time; fast turns switch to the sorted alphabetical section starts, update the unstyled native `digitalCrownAccessory` letter, and leave the selection on that section's first album. Reversing the Crown reverses both modes. Keep artwork clipped to its own viewport and the selected title in a background-free bottom lane so fast Crown input cannot leave partial covers over the title. Let the system own the accessory's appearance, placement, transient visibility, detents, and haptics. Tapping opens album detail, where album actions remain available. Do not add a page indicator, custom drag recognizer, manual scroll-offset tracking, or an album-cell `ScrollView`.
...
- Every Watch Play or Shuffle action and detail-track tap should start its scoped queue and present the single root-owned `WatchNowPlayingView` sheet. Keep the main page fixed and non-scrolling, use native top and bottom toolbar placements, and route transport and More actions to the existing local or iPhone playback owner. `WKInterfaceVolumeControl` owns Digital Crown volume for the selected target; do not replace it with `digitalCrownRotation` or a player-volume binding.
...
- Global toast window must stay above mini player and modal sheets for consistent feedback visibility; do not move the persistent root mini player into its own `UIWindow` to solve ordinary root layout issues.
...
- When `LargeScreenBrowseSplitView` collapses to a single inner pane, the selected detail must expose the shared back toolbar item that clears selection and returns to the browse list.
...
- Library browse screens that have already displayed cached content should not publish a transient empty list or swap to a full blank/loading state during refresh. Keep last-good rows visible, mark stale/loading locally when needed, and use stable placeholder rows only for the very first load before any cached content exists. Root library tabs should seed visible snapshots synchronously from cached published collections when the normal debounced display projections have not published yet. Playlists is the reference implementation.
...
- **Mini-player background:** The root mini player uses a system-color/material background instead of artwork blur because it remains visible under most phone surfaces and compounds every transition. On iPadOS the root mini player also avoids live Liquid Glass for its persistent shell so native split/sidebar transitions cannot duplicate material identity animations.

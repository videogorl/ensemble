...
- Watch Albums browse uses native discrete Digital Crown detents and a fixed two-cover stack, not a scrolling collection. Slow Crown turns replace albums one at a time; fast turns switch to the sorted alphabetical section starts, update the unstyled native `digitalCrownAccessory` letter, and leave the selection on that section's first album when the Crown becomes idle. Reversing the Crown reverses both modes. Keep artwork clipped to its own viewport and the selected title in a background-free bottom lane so fast Crown input cannot leave partial covers over the title. Let the system own the accessory's appearance, placement, and transient visibility. Tapping opens album detail, where album actions remain available. Do not add a page indicator, custom drag recognizer, manual scroll-offset tracking, or an album-cell `ScrollView`.

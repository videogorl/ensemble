...
- Watch Albums browse uses a native `ScrollView` and `LazyVStack` with full-height scroll targets. Each page contains maximized artwork; one full-width bottom title lane follows the native paging target. A native geometry visual effect pins passed covers into the same center stack while the interactive `scrollTransition` keeps the next cover below the resting viewport, then progressively raises and unfolds it from the bottom as the user scrolls. On watchOS 11 and later, bind a native `ScrollPosition`, hide the title lane during active scroll phases, and call `scrollTo(id:anchor: .top)` for its selected view at idle before revealing the title; watchOS 10 keeps the ID-bound pager and single title lane. Reversing scroll reverses the cover motion. The system owns Digital Crown input and target selection. Tapping opens album detail, where album actions remain available. Do not add a page indicator, custom drag recognizer, or manual scroll-offset tracking.

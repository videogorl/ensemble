...
- macOS viewport Now Playing should keep content in the root's native safe area while `NowPlayingBackdrop` handles edge-to-edge artwork/aurora bleed. The main scene owns `.windowResizability(.contentMinSize)` where available; `RootView` owns the main window's minimum content size and temporary window-toolbar suppression while viewport Now Playing is active, using a root-scoped AppKit toolbar bridge that preserves the window frame and restores the previous toolbar state. Do not add leaf-level titlebar or traffic-light padding constants, screen-level toolbar-item hiding, or fake full-window sheet shims.

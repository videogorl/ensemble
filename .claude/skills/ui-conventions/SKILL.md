...
- **Mini player offset:** Root chrome registration owns MiniPlayer placement. Use `TrackListLayoutMetrics.rootMiniPlayerBottomLift(safeAreaBottom:)` for tab-root chrome and `TrackListLayoutMetrics.detailMiniPlayerBottomLift(safeAreaBottom:)` for registered split/detail panes. Modern iOS keeps the standard floating lift while iOS 15 derives the root lift from the live tab/safe-area edge. Do not add per-screen mini-player offsets or animate root mini-player geometry across sidebar toggles, navigation pushes, or rotation. On iOS 26+, keep the persistent root mini-player shell out of shared Liquid Glass identity animation for unrelated navigation; Now Playing handoff animation should remain explicit.

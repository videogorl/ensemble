...
- **macOS detail header spacing:** Native macOS media detail tables use `EnsembleScaffold.DetailSurface.macWideHeaderTopDragRegion` plus `macWideHeaderBottomPadding` for the embedded wide header row. Keep the AppKit deterministic row-height calculation and the SwiftUI header spacing tokens in sync so albums, playlists, and merged playlists do not pick up content-dependent vertical padding. Pair the visual top region with `MediaDetailSurface`'s AppKit drag overlay so hidden-titlebar windows remain draggable without depending on arbitrary content padding.

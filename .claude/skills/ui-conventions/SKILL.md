...
- **iPad/macOS:** `ProfileToolbarButton` placed in sidebar toolbar, replacing the previous gear icon. On macOS, Downloads precedes Profile; the native menu uses a fixed toolbar footprint and shows the circular profile image when set, with the native profile glyph as its fallback.

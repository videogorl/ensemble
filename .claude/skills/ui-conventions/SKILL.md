...
- Every Watch Play or Shuffle action and detail-track tap should start its scoped queue and present the single root-owned sheet containing WatchKit's full-screen `NowPlayingView`. Do not wrap or overlay the system surface. Let it own source selection, metadata/artwork presentation, transport controls, and Digital Crown volume while Ensemble's playback owners continue publishing queue state and remote commands.

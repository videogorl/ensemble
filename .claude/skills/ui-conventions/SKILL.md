...
- Watch detail-track taps should navigate directly to the fixed Now Playing surface while starting the selected collection queue. Keep the surface non-scrolling, titleless, crown-volume driven, and composed from native top/bottom toolbar placements; use a small pre-rendered artwork wash instead of a live full-screen blur.

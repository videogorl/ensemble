...
- **Feed scrolling:** Do not add outer `DragGesture`/scroll-idle detectors around the Feed `ScrollView` to defer hub updates. Hidden Feed refreshes can wait until visibility returns, but visible hub snapshots should apply directly and let native scrolling own gesture arbitration. On macOS 26+, Feed's vertical scroll view should fill the top toolbar region and keep its first-row content inset with `EnsembleScaffold.Discovery.macToolbarContentInset`; this lets native Liquid Glass sample real scroll content at launch instead of showing a solid toolbar until the first push/pop navigation.

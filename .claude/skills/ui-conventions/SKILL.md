...
- **Lyrics scrolling:** Lyrics should rely on SwiftUI `ScrollView`/`TabView` native gesture arbitration. Do not reintroduce UIKit/AppKit drag or scroll-wheel detector probes, paging-pan failure wiring, or timed manual-scroll suppression unless a current simulator repro proves native behavior is broken and the smaller owner-scoped fix is documented.

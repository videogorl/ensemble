...
- On iOS 16+, navigation-bar background changes should use SwiftUI `.toolbarBackground(...)`; `NavigationBarAppearanceConfigurator` is an iOS 15-only fallback and must remain a no-op on modern iOS so UIKit appearance mutation does not fight native toolbar behavior.

...
- **Mini player offset:** Root chrome registration owns MiniPlayer placement. Use `TrackListLayoutMetrics.rootMiniPlayerBottomLift(safeAreaBottom:)` for tab-root chrome and `TrackListLayoutMetrics.detailMiniPlayerBottomLift(safeAreaBottom:)` for registered split/detail panes. Modern iOS keeps the standard floating lift while iOS 15 derives the root lift from the live tab/safe-area edge. Do not add per-screen mini-player offsets or publish root mini-player geometry from pushed destination content. iPadOS mini-player placement should resolve from root-space coordinates centered on the active content viewport: root bounds when the sidebar is closed, and root bounds minus the visible leading sidebar when the sidebar is open. Destination views and pushed detail content must not influence root mini-player placement; only sidebar visibility/width, root rotation/size, keyboard suppression, and Now Playing presentation should move or hide the persistent shell. Keep the persistent root mini player in the `RootSceneLayerHost` overlay across iPhone, iPadOS, and macOS so it shares the root view's rotation and safe-area coordinate space. On iOS 26+, keep the persistent iPadOS root mini-player shell on the stable system material surface and out of shared Liquid Glass identity animation for unrelated navigation; Now Playing handoff animation should remain explicit.

...
- On macOS, controls that belong to a custom selection pane (search, filter, sort) should live inside that pane when the pane is resizable. Window toolbar spacers do not track `LargeScreenBrowseSplitView`'s custom divider and can drift during resize.

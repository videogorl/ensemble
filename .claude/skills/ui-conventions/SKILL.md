...
- **Mini player offset:** Root chrome registration owns MiniPlayer placement. Use `TrackListLayoutMetrics.rootMiniPlayerBottomLift(safeAreaBottom:)` for tab-root chrome and `TrackListLayoutMetrics.detailMiniPlayerBottomLift(safeAreaBottom:)` for registered split/detail panes. Modern iOS keeps the standard floating lift while iOS 15 derives the root lift from the live tab/safe-area edge. Do not add per-screen mini-player offsets or publish root mini-player geometry from pushed destination content. iPadOS mini-player placement should resolve from root-space coordinates using the stable detail root frame plus `RootSidebarChromeRegistrationView` sidebar bounds, so navigation pushes cannot move the persistent shell. On iPadOS, render the persistent root mini player through `RootMiniPlayerWindowHost`; keep phone/macOS on the in-tree overlay unless their native transitions show the same bug. On iOS 26+, keep the persistent root mini-player shell on the stable system material surface and out of shared Liquid Glass identity animation for unrelated navigation; Now Playing handoff animation should remain explicit.
...
- Global toast window must stay above mini player and modal sheets for consistent feedback visibility; the iPad root mini-player overlay window stays below the toast window.
...
- **Mini-player background:** The root mini player uses a system-color/material background instead of artwork blur because it remains visible under most phone surfaces and compounds every transition. On iPadOS the window-hosted root mini player also avoids live Liquid Glass so native split/sidebar transitions cannot duplicate material shells.

...
- Artist detail keeps the full-width square hero on compact/collapsed layouts across platforms, then switches at wide widths to a media-detail-style header with circular artist artwork on the left and metadata/actions on the right. Its blurred artwork wash should render through `MediaDetailSurface` so Artist and album/playlist details share the same backdrop/chrome owner even when their header layouts differ.

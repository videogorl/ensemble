...
- **Lyrics adapter lifecycle:** Lyrics scroll/drag adapters should attach from representable update/window/superview/layout lifecycle callbacks. Do not add main-queue or timed retries to wait for the ScrollView hierarchy; keep detectors idempotent and detachable.
...
- On iOS 16+, navigation-bar background changes should use SwiftUI `.toolbarBackground(...)`; `NavigationBarAppearanceConfigurator` and root-level `UINavigationBar.appearance()` mutation are iOS 15-only fallbacks and must remain no-ops on modern iOS so UIKit appearance mutation does not fight native toolbar behavior.
...
- **Detail loading stability:** Album, playlist, artist, and virtual collection detail loads should avoid full-screen centered loaders that later swap to top-aligned headers. Use `MediaDetailSurface.LoadingState` or keep the shared header/table shell mounted with a compact footer loader during initial data fetches. On macOS, keep the detail root top-aligned, keep the native table's top content inset fixed at zero, let the AppKit backend use a deterministic wide-header row height, preload the initial album/playlist track snapshot in loaders before mounting the shared detail table, and publish virtual collection filtered snapshots before clearing their loading state so the first table layout already has its final rows; do not force the native table host itself to infinite height as a reflow workaround.
- **macOS detail header spacing:** Native macOS media detail tables should respect the window titlebar safe area; only the non-interactive artwork wash bleeds edge-to-edge. Use `EnsembleScaffold.DetailSurface.macWideHeaderTopPadding` plus `macWideHeaderBottomPadding` for the embedded wide header row, and keep the AppKit deterministic row-height calculation and SwiftUI header spacing tokens in sync so albums, playlists, and merged playlists do not pick up content-dependent vertical padding.

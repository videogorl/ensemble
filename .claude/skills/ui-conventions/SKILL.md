...
- Songs uses shared track-row visuals on large screens, with adaptive artist/album metadata columns when width allows. macOS hosts rows in a native SwiftUI `List` with `.trackSwipeActions(...)`; large-screen iPad hosts rows in `MediaTrackList` so UIKit owns `UISwipeActionsConfiguration` and avoids sidebar-width leading-swipe gaps. Do not use `TrackSwipeContainer` or reintroduce a column-customization table for Songs unless explicitly requested.

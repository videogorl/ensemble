...
- Every Watch Play or Shuffle action and detail-track tap should start its scoped queue and present the single root-owned Now Playing sheet. Keep the surface non-scrolling, titleless, and composed from native top/bottom toolbar placements; use a visible, focused `WKInterfaceVolumeControl` for system-owned local or companion Crown volume, a small pre-rendered artwork wash instead of a live full-screen blur, progress around Play/Pause from the active local or iPhone playback clock, and metadata clear of the native bottom transport toolbar on compact displays.

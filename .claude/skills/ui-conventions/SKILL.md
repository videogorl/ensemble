...
- On macOS, SwiftUI toolbar actions that need to sit to the right of a search field should use one `ToolbarItem { Spacer() }` before the action group. Do this as a toolbar-level alignment spacer, not as column-width math or screen-level toolbar delegate proxying.

...
- **Detail toolbar bleed:** Artwork-backed detail roots should apply `artworkDetailToolbarBleed()` and top safe-area bleed from the content owner so Artist and Media detail headers use the same native toolbar/titlebar behavior. Do not add titlebar drag bridges or AppKit toolbar proxies to leaf detail views.

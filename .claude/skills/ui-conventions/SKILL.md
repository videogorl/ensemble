...
- Watch Albums browse uses a native `ScrollView` and `LazyVStack` with full-height scroll targets. Each album contains only maximized artwork and title. A native geometry visual effect pins passed covers into the same center stack while the interactive `scrollTransition` keeps the next cover below the resting viewport, then progressively raises and unfolds it from the bottom as the user scrolls. The title follows the native paging scroll-position ID in a noninteractive overlay above the cover layer, keeping it visible during high-velocity transitions. Keep that ID binding anchorless; an explicit center anchor conflicts with full-page Watch settlement and can park the view between covers. Reversing scroll reverses the cover motion. The system owns Digital Crown scrolling and page settling. Tapping opens album detail, where album actions remain available. Do not add a page indicator, custom drag recognizer, or manual scroll-offset tracking.

...
- Watch Albums browse uses a native `ScrollView` and `LazyVStack` with full-height scroll targets. Each album contains only artwork and title, and an interactive `scrollTransition` hinges the card around its bottom edge while the system owns Digital Crown scrolling and page settling. Tapping opens album detail, where album actions remain available. Do not add a page indicator, custom drag recognizer, or manual scroll-offset tracking.

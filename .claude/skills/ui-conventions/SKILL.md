...
- Every Watch Play or Shuffle action and detail-track tap should navigate directly to the fixed Now Playing surface while starting the scoped queue. Keep the surface non-scrolling, titleless, crown-volume driven, and composed from native top/bottom toolbar placements; use a small pre-rendered artwork wash instead of a live full-screen blur, surround Play/Pause with progress from the active local or iPhone playback clock, and keep metadata clear of the native bottom transport toolbar on compact displays.

...
- Refreshable root screens should attach `.refreshable { ... }` directly to the native vertical scroll owner for every visible content state, including empty/error states that mention pull-to-refresh. Also attach `.refreshCommand { ... }`, which publishes through the scene-focused refresh action so macOS View > Refresh and iPadOS keyboard/menu commands invoke the active screen's same async refresh action without requiring a child control to hold focus.

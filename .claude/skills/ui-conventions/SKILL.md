...
- Watch Albums browse uses a native `ScrollView` and `LazyVStack` with full-height scroll targets. Each album contains only maximized artwork and title. A native geometry visual effect pins passed covers into the same center stack while the interactive `scrollTransition` raises and unfolds the incoming cover from the bottom, then reverses that motion when scrolling back. The system owns Digital Crown scrolling and page settling. Tapping opens album detail, where album actions remain available. Do not add a page indicator, custom drag recognizer, or manual scroll-offset tracking.

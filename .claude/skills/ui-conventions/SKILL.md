...
- **Shared detail actions:** Detail Play/Shuffle-style button labels should use `MediaDetailSurface.ActionLabel`, repeated Play/Shuffle action strips should use `MediaDetailSurface.ActionRow`/`PlaybackActionRow`, nested compact sections should use `CompactPlaybackActionRow`, and icon-only actions such as Radio should use `IconActionLabel` so filled/accent, secondary action, spacing, disabled state, and chromeless button treatment stay aligned across media detail variants.

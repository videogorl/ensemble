...
- **Artwork-backed toolbar bleed:** Artwork-backed roots such as Feed, MediaDetail, and Artist detail should apply `artworkBackedToolbarBleed()` from the surface owner so artwork and scrolling content can remain visible behind native translucent toolbar chrome. Keep the main macOS window on the native titlebar style; hide only the toolbar background on supported macOS releases, keep AppKit scroll/clip views transparent, and do not add titlebar drag bridges or AppKit toolbar proxies to leaf views.

...
- Low Data Mode pauses downloads on constrained network paths without treating the device as offline; pending work resumes when the path is no longer constrained. The user may press Resume to allow downloads for one hour on Low Data Mode or cellular when the persistent cellular setting is off. This temporary exception does not change settings, cannot override an offline path, and reapplies normal policy when it expires.

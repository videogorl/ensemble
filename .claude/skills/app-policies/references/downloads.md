...
- Download lookup, persistence, and deletion must be source-aware: use `ratingKey + sourceCompositeKey` so libraries and servers do not collide. Library download rows remain account-scoped; when multiple accounts expose the same server and library names, keep separate rows and include the account identifier in their visible titles.

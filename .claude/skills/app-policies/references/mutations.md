...
- Item mutation affordances resolve through Core `MusicItemActionAvailability` as available, read-only, or unavailable with an action-specific reason. A provider may supply per-item `MusicItemActionCapabilities`; omitted actions retain source-wide defaults. Shared menus and source pickers consume that contract instead of branching on Plex, Apple Music, or future provider types. Missing or malformed source ownership never inherits Plex permissions.

...
- Playlist display merging is provider-agnostic by normalized title and smart status. Mutations split to compatible constituents: Apple playlists accept only Apple tracks and Plex playlists retain server compatibility. Apple Music playlist/favorite mutations are online-only and never enter the Plex pending-mutation queue. MusicKit owns Apple playlist create/add/rename/rebuild; playlist deletion remains visibly unavailable because MusicKit exposes no supported delete operation.

...
- Feed merges Recently Added, Recent Plays, and Most Played hubs across all enabled Plex libraries and servers. Recently Added and Recent Plays interleave items newest-first using Plex `addedAt` and `lastViewedAt`; Most Played orders by Plex `viewCount`, then `lastViewedAt` for ties. Merged hubs deduplicate the same Plex rating key from account aliases of one physical server and library, while preserving matching rating keys from different servers or library sections. Other hubs remain server-scoped. Feed hub ordering is global across the combined snapshot and must survive cached, partial, and refreshed payloads through stable merged-hub identities.

...
- Feed merges Recently Added, Recent Plays, and Most Played hubs across all enabled Plex libraries and servers. Recently Added and Recent Plays interleave items newest-first using Plex `addedAt` and `lastViewedAt`; Most Played orders by Plex `viewCount`, then `lastViewedAt` for ties. Other hubs remain server-scoped.

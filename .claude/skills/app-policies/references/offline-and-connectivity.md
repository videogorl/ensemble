...
- Album, artist, and playlist artwork is part of the offline library cache. Artwork resolved during sync or visible online browsing should be persisted to the local artwork cache and reused across relaunch/offline sessions; Nuke's image cache is only a rendering/performance layer and is not the source of truth for offline artwork. Artwork metadata/path invalidation should mark persistent artwork stale for online refresh, not delete the file before a replacement exists; stale persistent artwork may be shown while offline or while the server is unavailable.

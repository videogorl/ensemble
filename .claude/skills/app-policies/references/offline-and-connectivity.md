...
- When source restoration newly enables a server, preserve one follow-up health pass after any in-flight pass completes. Unrelated source-settling updates must not cancel it, startup/foreground/inventory health passes must not overlap, and an earlier pass over a partial source configuration must not leave the restored server indefinitely unknown.

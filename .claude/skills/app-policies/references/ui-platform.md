...
- Media detail screens should render cached header/list content immediately. Do not hide cached detail content behind artificial reveal delays, opacity fades, or supplemental metadata loads; richer metadata may fill in after the base detail surface is interactive.
- Playlist detail navigation should not load or map full track relationships before rendering the detail surface. If tracks are still loading, show that state inside the track list/footer so the header and toolbar remain stable.

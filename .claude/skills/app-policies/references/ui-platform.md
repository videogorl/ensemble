...
- Detail screens with artwork-backed washes should use shared detail surface and toolbar-bleed owners so media details, download details, and artist details do not drift. Media detail backdrops should keep the previous resolved artwork wash while the next detail artwork is unavailable, then cross-fade the wash only when the new artwork arrives; artist detail hero images should avoid nil-to-image fade-ins and should cross-fade between resolved artist images during detail navigation. Content should still render immediately without artificial reveal delays.

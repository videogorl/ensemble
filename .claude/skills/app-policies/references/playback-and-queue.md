...
- Watch-local playback keeps one in-memory indexed queue for the selected detail, advances automatically when a track ends, and cancels stale stream requests when the user changes tracks or queues. Regular play starts at the requested track; Shuffle randomizes the full queue before playback. Previous restarts after three seconds or moves to the prior queue entry near the start, including duplicate entries; Next does not wrap past the final track.

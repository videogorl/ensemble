...
- On iOS/iPadOS 18+, Apple Music tracks remain in that same Ensemble queue, but contiguous Apple Music runs play through `ApplicationMusicPlayer`. Stop one provider before crossing an Apple Music/Plex boundary; never decode, export, analyze, or mix Apple Music PCM with Plex audio.
- MusicKit owns Apple-to-Apple gapless behavior, AirPlay routing, and SmartMix crossfade (`ApplicationMusicPlayer.transition`). Ensemble's local two-deck SmartMix remains Plex-only. Apple Music tracks publish an empty waveform/frequency snapshot and do not expose Lyrics or instrumental mode.

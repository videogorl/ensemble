...
- On iOS/iPadOS 18+, Apple Music tracks remain in that same Ensemble queue, but contiguous Apple Music runs play through `ApplicationMusicPlayer`. Resume must rebuild the MusicKit queue from Ensemble's restored queue when the native queue is no longer available. Stop one provider before crossing an Apple Music/Plex boundary; never decode, export, analyze, or mix Apple Music PCM with Plex audio.
- MusicKit owns Apple-to-Apple gapless behavior, AirPlay routing, SmartMix crossfade (`ApplicationMusicPlayer.transition`), and audio-session interruptions while it is active. Ensemble's local two-deck SmartMix remains Plex-only. Apple Music tracks publish an empty waveform/frequency snapshot and do not expose Lyrics or instrumental mode.
...
- Apple Music autoplay starts the seed song's MusicKit station when an Apple run exhausts. Plex may fall back to Apple Music only after one exact normalized artist/title catalog match; ambiguous matches stop normally.
...
- Device-offline queues are filtered to downloaded tracks. Device-online queues skip non-downloaded tracks from known-unavailable servers; unknown or in-flight health states must not clear or strand an existing queue.

...
- Active incremental stream failures, including sustained post-start PCM starvation, must stop silent rendering and restart the current queue item from its visible playhead; intentional stream cancellation during queue changes must not trigger recovery.

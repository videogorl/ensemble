...
- SmartMix uses bounded local/temp file silence and tempo analysis, equal-power overlapping deck fades, a subtle outgoing high-pass sweep, and confidence-gated incoming-only time stretching. Tempo matching is limited to subtle close-tempo changes and automatically falls back to non-stretched SmartMix when confidence or device state is not suitable.

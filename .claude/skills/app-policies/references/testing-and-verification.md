...
- Plex sync/download changes require focused automated proof that failed or incomplete inventories preserve last-good data, plus explicitly authorized disposable-fixture proof for foreground and cold-start additions, metadata edits, downloaded-playlist membership changes, and authoritative removal cleanup. The live matrix must record server state, convergence timing, exact source-scoped database/files, visible UI, no-op polling cost, and complete fixture cleanup; it never authorizes mutation of existing Plex media or playlists.

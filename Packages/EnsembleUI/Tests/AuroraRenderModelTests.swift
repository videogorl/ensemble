...
final class AuroraRenderModelTests: XCTestCase {
    func testDisplaySamplingTrimsSparseLowsAndMirrorsActiveRangeWithWidth() {
        ...
        for (width, upperIndex) in [(390.0, 18.0), (665.0, 20.5), (900.0, 23.0), (1200.0, 23.0)] {
            for count in [24, 48] {
                let samples = model.displayBands(width: width, count: count)
                XCTAssertEqual(samples.count, count)
                let edgeValue = (source[Int(floor(upperIndex))] + source[Int(ceil(upperIndex))]) / 2
                XCTAssertEqual(samples.first!, edgeValue)
                XCTAssertEqual(samples.last!, edgeValue)
                XCTAssertEqual(samples[count / 2 - 1], source[8])
                XCTAssertEqual(samples[count / 2], source[8])
    ...
    func testAttackAndReleasePreserveDynamicsAcrossUpdateRates() {
        for rate in [15, 30, 60] {
            ...
            for tick in 1...(rate / 5) {
                model.advance(targetBands: Array(repeating: 0.8, count: 24), at: Double(tick) / Double(rate))
            }
            let peak = model.renderedBands[12]
            XCTAssertEqual(peak, 0.8 + (initial - 0.8) * exp(-0.2 / 0.03), accuracy: 0.000001)
            for tick in 1...rate {
                model.advance(targetBands: quiet, at: 0.2 + Double(tick) / Double(rate))
            }

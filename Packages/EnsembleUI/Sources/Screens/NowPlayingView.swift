        ...
        VStack(spacing: 0) {
            ...
            if !viewModel.queue.isEmpty {
                ...
                if !upNext.isEmpty {
                    ...
                    // Build set of auto-generated track IDs
                    let autoGenIds = Set(displayedTracks.filter { viewModel.isTrackAutoGenerated($0.id) }.map { $0.id })
                    
                    TrackListView(
                        tracks: displayedTracks,
                        showArtwork: true,
                        autoGeneratedTrackIds: autoGenIds
                    ) { track, index in

...
final class AutoplayTests: XCTestCase {
    ...
    func testAutoplayKeepsDuplicateRatingKeysFromDifferentSourcesDistinct() {
        let qm = QueueManager()
        qm.setQueue(
            tracks: [makeTrack("seed", sourceCompositeKey: "plex:subscriber:server:music")],
            startingAt: 0
        )
        qm._setQueueState(queue: qm.queue, currentIndex: 0, isAutoplayEnabled: true)

        let subscriberTrack = makeTrack("7551", sourceCompositeKey: "plex:subscriber:server:music")
        let freeAccountTrack = makeTrack("7551", sourceCompositeKey: "plex:free:server:music")
        let added = qm.addAutoplayTracks([subscriberTrack, freeAccountTrack])

        XCTAssertEqual(added.map(\.playbackIdentity), [
            subscriberTrack.playbackIdentity,
            freeAccountTrack.playbackIdentity
        ])
        XCTAssertTrue(qm.isTrackAutoGenerated(trackId: subscriberTrack.playbackIdentity))
        XCTAssertTrue(qm.isTrackAutoGenerated(trackId: freeAccountTrack.playbackIdentity))
    }
    ...
    func testAutoplayTrimRemovesSourceScopedGeneratedIdentities() {
        let qm = QueueManager(maxQueueLookahead: 2)
        qm.setQueue(
            tracks: [makeTrack("seed", sourceCompositeKey: "plex:subscriber:server:music")],
            startingAt: 0
        )
        ...
        let keptSubscriber = makeTrack("7551", sourceCompositeKey: "plex:subscriber:server:music")
        let keptFree = makeTrack("7551", sourceCompositeKey: "plex:free:server:music")
        let trimmedSubscriber = makeTrack("7552", sourceCompositeKey: "plex:subscriber:server:music")
        let trimmedFree = makeTrack("7552", sourceCompositeKey: "plex:free:server:music")
        qm.addAutoplayTracks([keptSubscriber, keptFree, trimmedSubscriber, trimmedFree])

        XCTAssertTrue(qm.isTrackAutoGenerated(trackId: keptSubscriber.playbackIdentity))
        XCTAssertTrue(qm.isTrackAutoGenerated(trackId: keptFree.playbackIdentity))
        XCTAssertFalse(qm.isTrackAutoGenerated(trackId: trimmedSubscriber.playbackIdentity))
        XCTAssertFalse(qm.isTrackAutoGenerated(trackId: trimmedFree.playbackIdentity))
    }
...
final class HistoryTests: XCTestCase {
    ...
    func testPlayFromHistoryDistinguishesDuplicateRatingKeysBySource() {
        let qm = QueueManager()
        let subscriberTrack = makeTrack("7551", sourceCompositeKey: "plex:subscriber:server:music")
        let freeAccountTrack = makeTrack("7551", sourceCompositeKey: "plex:free:server:music")
        let queue = [
            QueueItem(track: subscriberTrack, source: .continuePlaying),
            QueueItem(track: freeAccountTrack, source: .continuePlaying)
        ]
        let history = [
            QueueItem(track: freeAccountTrack, source: .continuePlaying)
        ]
        qm._setQueueState(queue: queue, currentIndex: 0, history: history)
        ...
        if case .playIndex(let index) = action {
            ...
            XCTFail("Expected .playIndex for the matching source-scoped track")
        }
    }

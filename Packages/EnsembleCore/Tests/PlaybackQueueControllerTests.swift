...
final class PlaybackQueueControllerTests: XCTestCase {
    ...
    func testQueueForPersistenceRemovesOnlyFutureAutoplayItems() {
        let queue = [
            ...
            QueueItem(id: "manual", track: makeTrack(id: "manual"), source: .upNext),
            QueueItem(id: "generated", track: makeTrack(id: "generated"), source: .autoplay)
        ]

        let persisted = PlaybackQueueController.queueForPersistence(queue, currentItemID: "current")

        XCTAssertEqual(persisted.map(\.id), ["current", "manual"])
    }

...
public final class DependencyContainer: @unchecked Sendable {
    ...
    private init() {
        ...
        watchConnectivityCoordinator = MainActor.assumeIsolated { WatchConnectivityCoordinator() }
    ...
    private func wireCrossSubsystemCallbacks() {
        MainActor.assumeIsolated {
            ...
            wireWatchConnectivityCallbacks()
        }
    }

    @MainActor
    private func wireWatchConnectivityCallbacks() {
        let publishSnapshot = { [weak watchConnectivityCoordinator, weak playbackService] in
            guard let watchConnectivityCoordinator, let playbackService else { return }
            let snapshot = WatchRemoteSessionSnapshot(
                currentTrack: playbackService.currentTrack,
                playbackState: WatchPlaybackState(playbackService.playbackState),
                playbackError: {
                    if case .failed(let message) = playbackService.playbackState {
                        return message
                    }
                    return nil
                }(),
                ...
                repeatModeRawValue: playbackService.repeatMode.rawValue,
                sourceName: "iPhone"
            )
            watchConnectivityCoordinator.publishRemoteSnapshot(snapshot)
        }

        watchConnectivityCoordinator.syncCredentialProvider = { [weak accountManager] in
            accountManager?.exportSyncCredentials() ?? []
        }
        ...
        watchConnectivityCoordinator.commandHandler = { [weak playbackService, weak watchConnectivityCoordinator] command in
            guard let playbackService, let watchConnectivityCoordinator else {
            ...
            case .playTracks:
                ...
                await playbackService.play(tracks: command.tracks, startingAt: command.startingIndex ?? 0)

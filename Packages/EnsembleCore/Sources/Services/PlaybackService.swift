...
/// The playback engine that owns one continuous queue session.
enum PlaybackQueueEngine: String, Sendable {
    case ensemble
    case appleMusic
}
...
public protocol PlaybackServiceProtocol: AnyObject {
    ...
    var incompatibleQueueItemCount: Int { get }
    ...
    var incompatibleQueueItemCountPublisher: AnyPublisher<Int, Never> { get }
    ...
    func queueActionAvailability(for tracks: [Track]) -> MusicItemActionAvailability
...
public extension PlaybackServiceProtocol {
    ...
    var incompatibleQueueItemCount: Int { 0 }

    var incompatibleQueueItemCountPublisher: AnyPublisher<Int, Never> {
        Just(0).eraseToAnyPublisher()
    }

    func queueActionAvailability(for tracks: [Track]) -> MusicItemActionAvailability {
        tracks.isEmpty
            ? .unavailable(reason: "No tracks are available for this action.")
            : .available
    }
}
...
public final class PlaybackService: NSObject, PlaybackServiceProtocol {
    ...
    struct QueueEngineAffinityResult: Equatable {
        let queue: [QueueItem]
        let currentIndex: Int
        let removedItemCount: Int
        let engine: PlaybackQueueEngine?
    }

    static func queueEngine(for track: Track) -> PlaybackQueueEngine {
        track.isAppleMusic ? .appleMusic : .ensemble
    }

    static func queueActionAvailability(
        for tracks: [Track],
        activeEngine: PlaybackQueueEngine?
    ) -> MusicItemActionAvailability {
        guard !tracks.isEmpty else {
            return .unavailable(reason: "No tracks are available for this action.")
        }
        guard let activeEngine else { return .available }
        return tracks.contains { queueEngine(for: $0) == activeEngine }
            ? .available
            : .unavailable(reason: "This item uses a different playback engine than the current queue.")
    }

    static func enforceQueueEngineAffinity(
        queue: [QueueItem],
        currentIndex: Int
    ) -> QueueEngineAffinityResult {
        guard queue.indices.contains(currentIndex) else {
            return QueueEngineAffinityResult(
                queue: queue,
                currentIndex: queue.isEmpty ? -1 : min(max(currentIndex, 0), queue.count - 1),
                removedItemCount: 0,
                engine: nil
            )
        }

        let currentItemID = queue[currentIndex].id
        let engine = queueEngine(for: queue[currentIndex].track)
        let filteredQueue = queue.filter { queueEngine(for: $0.track) == engine }
        let filteredIndex = filteredQueue.firstIndex { $0.id == currentItemID } ?? 0
        return QueueEngineAffinityResult(
            queue: filteredQueue,
            currentIndex: filteredIndex,
            removedItemCount: queue.count - filteredQueue.count,
            engine: engine
        )
    }
    ...
    static func pruningRestoredSnapshot(
        ...
        let sourceResult = pruneQueueForSourceConfiguration(
        ...
        let affinityResult = enforceQueueEngineAffinity(
            queue: sourceResult.queue,
            currentIndex: sourceResult.nextCurrentQueueIndex
        )
        let history = affinityResult.engine.map { engine in
            sourceResult.playbackHistory.filter { queueEngine(for: $0.track) == engine }
        } ?? sourceResult.playbackHistory
        let removedCurrentItem = sourceResult.removedCurrentQueueItem
            || affinityResult.currentIndex < 0
        return PlaybackQueueSnapshot(
            queue: affinityResult.queue,
            history: history,
            currentIndex: affinityResult.currentIndex,
            currentTime: removedCurrentItem ||
                !affinityResult.queue.indices.contains(affinityResult.currentIndex)
                ? 0
                : snapshot.currentTime,
            hasUserQueueEdits: snapshot.hasUserQueueEdits,
            incompatibleQueueItemCount: snapshot.incompatibleQueueItemCount
                + affinityResult.removedItemCount
        )
    }
    ...
    public var incompatibleQueueItemCountPublisher: AnyPublisher<Int, Never> {
        $incompatibleQueueItemCount.eraseToAnyPublisher()
    }
    ...
    private func setupPlayer() {
        ...
        engine.onSmartMixTransitionActiveChanged = { [weak self] isActive in
            DispatchQueue.main.async {
                guard let self, self.isSmartMixTransitionActive != isActive else { return }
    ...
    public func play(tracks: [Track], startingAt index: Int, context: PlaybackStartContext) async {
        ...
        guard let playableQueue = await resolvePlayableQueue(tracks: tracks, preferredStartIndex: index) else {
            ...
            await stop()
            let affinity = Self.enforceQueueEngineAffinity(
                queue: tracks.map { makeQueueItem(track: $0, source: .continuePlaying) },
                currentIndex: index
            )
            queue = affinity.queue
            originalQueue = queue
            currentQueueIndex = affinity.currentIndex
            incompatibleQueueItemCount = affinity.removedItemCount
            currentTrack = queue.indices.contains(currentQueueIndex)
        ...
        let affinity = Self.enforceQueueEngineAffinity(
            queue: playableQueue.tracks.map { makeQueueItem(track: $0, source: .continuePlaying) },
            currentIndex: playableQueue.startIndex
        )
        let queueTracks = affinity.queue.map(\.track)
        ...
        UserJourneyLogger.log(
            ...
            details: [
                ...
                "startIndex": "\(affinity.currentIndex)",
                "incompatibleRemoved": "\(affinity.removedItemCount)"
            ]
        )
        savePlaybackState()
        await donatePlaybackStartIfNeeded(
            context: context,
            fallbackTrack: queueTracks[affinity.currentIndex],
            shuffle: false
        )
    ...
    public func shufflePlay(tracks: [Track], context: PlaybackStartContext) async {
        ...
        guard let playableQueue = await resolvePlayableQueue(tracks: tracks, preferredStartIndex: 0) else {
            await stop()
            var shuffled = tracks.map { makeQueueItem(track: $0, source: .continuePlaying) }
            shuffled.shuffle()
            let affinity = Self.enforceQueueEngineAffinity(queue: shuffled, currentIndex: 0)
            queue = affinity.queue
            originalQueue = queue
            currentQueueIndex = affinity.currentIndex
            incompatibleQueueItemCount = affinity.removedItemCount
            currentTrack = queue.first?.track ?? tracks[0]
            let isDeviceOffline = await MainActor.run {
                !networkMonitor.networkState.isConnected || syncCoordinator.isOffline
            }
            playbackState = .failed(Self.noPlayableTracksMessage(isDeviceOffline: isDeviceOffline))
        ...
        shuffled.shuffle()
        let affinity = Self.enforceQueueEngineAffinity(queue: shuffled, currentIndex: 0)
        let engine = affinity.engine
        originalQueue = engine.map { activeEngine in
            items.filter { Self.queueEngine(for: $0.track) == activeEngine }
        } ?? affinity.queue
        queue = affinity.queue
        currentQueueIndex = affinity.currentIndex
        incompatibleQueueItemCount = affinity.removedItemCount
        ...
        playbackHistory.removeAll()
        queueController.clearAutoGeneratedTrackIds()
        let newTrackIds = Set(queue.map(\.track.playbackIdentity))
        await MainActor.run { evictPlayerItemsNotIn(newTrackIds) }
    ...
    public func queueActionAvailability(for tracks: [Track]) -> MusicItemActionAvailability {
        Self.queueActionAvailability(for: tracks, activeEngine: activeQueueEngine)
    }

    private var activeQueueEngine: PlaybackQueueEngine? {
        guard queue.indices.contains(currentQueueIndex) else { return nil }
        return Self.queueEngine(for: queue[currentQueueIndex].track)
    }

    private func compatibleTracksForActiveQueue(_ tracks: [Track]) -> [Track] {
        guard let activeQueueEngine else { return tracks }
        return tracks.filter { Self.queueEngine(for: $0) == activeQueueEngine }
    }

    private func recordIncompatibleQueueItems(_ count: Int) {
        guard count > 0 else { return }
        incompatibleQueueItemCount += count
        EnsembleLogger.playback(
            "QUEUE_AFFINITY: removed=\(count), totalRemoved=\(incompatibleQueueItemCount), engine=\(activeQueueEngine?.rawValue ?? "none")"
        )
    }
    ...
    public func playNext(_ tracks: [Track]) {
        guard !tracks.isEmpty else { return }
        let compatibleTracks = compatibleTracksForActiveQueue(tracks)
        recordIncompatibleQueueItems(tracks.count - compatibleTracks.count)
        guard !compatibleTracks.isEmpty else {
            savePlaybackState()
            return
        }
        let items = compatibleTracks.map { makeQueueItem(track: $0, source: .upNext) }
    ...
    public func playLast(_ tracks: [Track]) {
        ...
        let items = compatibleTracks.map { makeQueueItem(track: $0, source: .continuePlaying) }
    ...
    public func refreshAutoplayQueue() async {
        ...
        if let tracks = recommendations {
            ...
            let uniqueNewTracks = compatibleTracksForActiveQueue(uniqueTracks)
            recordIncompatibleQueueItems(uniqueTracks.count - uniqueNewTracks.count)
    ...
    public func enableRadio(tracks: [Track]) async {
        ...
        items.shuffle()
        let affinity = Self.enforceQueueEngineAffinity(queue: items, currentIndex: 0)
        items = affinity.queue
    ...
    private func setupAppleMusicPlayback() {
        ...
        private func isCurrentAppleMusicQueue(_ queueGeneration: UInt64) -> Bool {
            Self.shouldAcceptAppleMusicCallback(
                queueGeneration: queueGeneration,

...
public final class PlaybackService: NSObject, PlaybackServiceProtocol {
    ...
    private var hasTriggeredDurationBoundary = false  // Prevents re-triggering end-of-track enforcement
        ...
        if let pair = playerItems.first(where: { $0.value === item }) {
            ...
            if let index = queue.firstIndex(where: { $0.track.id == ratingKey }) {
                if currentQueueIndex != index {
                    ...
                    hasTriggeredDurationBoundary = false
    ...
    private func setupObservers(for item: AVPlayerItem) {
        ...
        timeObserver = player?.addPeriodicTimeObserver(
            ...
            // Enforce duration boundary: after seeking in HTTP progressive
            // downloads, AVPlayer can lose its end-of-item notification and
            // play audio indefinitely past the known duration. Clamp the
            // displayed time and trigger track advance when this happens.
            if effectiveDur > 0 && rawTime >= effectiveDur {
                self.currentTime = effectiveDur
                if !self.hasTriggeredDurationBoundary && rawTime >= effectiveDur + 0.5 {
                    self.hasTriggeredDurationBoundary = true
                    #if DEBUG
                    EnsembleLogger.debug("SCRUBBER_DIAG: duration boundary enforced — rawTime=\(String(format: "%.1f", rawTime))s, duration=\(String(format: "%.1f", effectiveDur))s, advancing track")
                    #endif
                    Task { [weak self] in
                        await self?.handleQueueExhausted()
                    }
            ...
            if !self.hasLoggedDurationOverrun,
               ...
               let track = self.currentTrack {
                ...
                EnsembleLogger.debug("SCRUBBER_DIAG: track='\(track.title)' metadata=\(String(format: "%.2f", metaDur))s item=\(itemStr)s effective=\(String(format: "%.2f", effectiveDur))s")
            }
            #endif
            self.updateBufferedProgress()
            self.updateNowPlayingProgress()
